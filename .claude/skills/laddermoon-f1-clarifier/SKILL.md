...
2. **Search the repository for answers**
   ...
   Based on the question category, search appropriate sources:

   | Category | Where to Search |
   |----------|----------------|
   | `missing-overview` | README.md, package.json, go.mod, main entry files |
   | `incomplete-structure` | Directory structure, module files, imports |
   | `missing-index` | docs/, configs, key directories |
   | `missing-source` | The claim's likely source files |
   | `unverifiable` | Implementation files that should contain the feature |
   | `project-concern` | Documentation files, configs, manifests |
   ...
   grep -r "<keyword>" --include="*.md" --include="*.json"
